...
function ProductVariationComponent({ form }: { form: any }) {
  ...
  const handleUpdateVariation = useCallback(() => {
    ...
    form.setValue(
      "productVariations",
      variations.map((v: ProductVariation) => (v.id === currentVariation.id ? currentVariation : v)),
      { shouldValidate: true },
    )
  ...
  const calculateDiscountAmount = (price: number, discountedPrice: number) => {
    return Number((price - discountedPrice).toFixed(2))
  }
    ...
    <div className="w-full px-4">
          ...
          {form.watch("productVariations")?.map((variation: ProductVariation) => (
            <Card key={variation.id}>
              <CardContent className="p-4">
                ...
                <p>Discount: ₹ {calculateDiscountAmount(variation.price, variation.discountedPrice)}</p>
      ...
      {form.formState.errors.productVariations && (
        <Alert variant="destructive" className="mt-4">
          ...
          <AlertDescription>{form.formState.errors.productVariations.message}</AlertDescription>
        </Alert>
      )}
    </div>
  )
