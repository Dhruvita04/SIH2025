...
import type { FormValues, ProductFormProps, ProductVariationEdit } from "./types"
...
const formSchema = z.object({
  ...
  productImages: z.array(z.instanceof(File)).default([]),
  hasAlternativeProduct: z.boolean(),
  productAlternatives: z.preprocess((val) => val, z.union([productAlternativeSchema, z.object({}).optional()])),
  productAlternativeImage: z.union([z.instanceof(File), z.null()]).optional(),
})
...
export default function ProductForm({ isEditMode = false }: ProductFormProps) {
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: {
      ...
      productAlternatives: {
        ...
        productAlternativeDiscountType: "fixed",
  ...
  const productMutation = useMutation({
    mutationFn: (payload: Record<string, any>) => {
      ...
      const variations = payload.productVariations.map((variation: any) => {
        const discountAmount = variation.price - variation.discountedPrice
        return {
          ...variation,
  ...
  async function onSubmit(values: FormValues) {
    try {
      ...
      const payload: Record<string, any> = {
        ...
        productHsnCode: values.productHsnCode,
        tags: values.tags || [],
      ...
      if (!payload.productSlug || payload.productSlug.trim() === "") {
        const generatedSlug = values.productName
          .toLowerCase()
          .replace(/[^a-z0-9]+/g, "-")
          .replace(/(^-|-$)/g, "")
        payload.productSlug = generatedSlug
      }
  ...
  useEffect(() => {
    ...
    if (productData?.data.data) {
      ...
      const loadFormData = async () => {
        const productImages = productDetails.isPlaceholder ? [] : await loadImagesAsFiles(productDetails.images || [])
        const productAlternativeImage =
          alternativeDetails?.imageUrl && !productDetails.isPlaceholder
        ...
        const formValues = {
          ...
          productHsnCode: productDetails.hsnCode || "",
          tags: productDetails.tags || [],
          productVariations:
            variationsDetails?.map((variation: ProductVariationEdit) => ({
              id: variation.id,
              name: variation.name,
              price: variation.price,
              discountedPrice:
                variation.discountType === "fixed"
          ...
          productAlternatives: {
            ...
            productAlternativeDiscountType: alternativeDetails?.discountType || "fixed",
  ...
  return (
    <div
      className={`w-full h-[calc(100%-1.5rem)] overflow-y-auto rounded-lg ${isDarkMode ? "bg-zinc-900" : "bg-white"}`}
    >
      {isEditMode && (
        <div className="p-4">
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="w-full h-full p-4">
          <div className="w-full flex flex-col lg:flex-row gap-4">
            <div className="w-full lg:w-[65%] flex flex-col gap-4">
              <Card className="w-full h-fit">
                <CardHeader className="p-[0.75rem]">
                  <CardTitle>Product Main Details</CardTitle>
              ...
              <Card className="w-full h-fit">
                <CardHeader className="p-[0.75rem]">
                  <CardTitle>{formSingular} Variations</CardTitle>
                </CardHeader>
            ...
            <div className="w-full lg:w-[35%] flex flex-col gap-4">
              ...
              <Card className="w-full h-fit">
                <CardHeader className="p-[0.75rem]">
                  <CardTitle>Branded Alternative</CardTitle>

                  <CardContent className="p-0 pt-4">
                    <div className="flex items-center gap-2">
                      <Switch
                        id="has-alternative-product"
                        checked={hasAlternativeProduct}
                        onCheckedChange={handleAlternativeProductChange}
                      />
