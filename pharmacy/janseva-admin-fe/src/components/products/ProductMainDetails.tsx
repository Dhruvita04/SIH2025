  ...
  const loadProductDetails = (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    if (form.getValues("productName")) {
      ...
      productDetailsMutation.mutate({
        medicineName: form.getValues("productName"),
      })
    } else {
      ...
      try {
        ...
        if (status === "success") {
          ...
          if (data.additionalInformation?.length > 0) {
            const additionalInformation = data.additionalInformation.map((info: string) => `<li>${info}</li>`).join("")
          ...
          if (data.ingredients?.length > 0) {
            const ingredients = data.ingredients.map((ingredient: string) => `<li>${ingredient}</li>`).join("")
          ...
          if (data.precaution?.length > 0) {
            const precaution = data.precaution.map((step: string) => `<li>${step}</li>`).join("")
          ...
          if (data.interactions?.length > 0) {
            const interactions = data.interactions.map((interaction: string) => `<li>${interaction}</li>`).join("")
          ...
          if (data.dosageInformation?.length > 0) {
            const dosageInformation = data.dosageInformation.map((step: string) => `<li>${step}</li>`).join("")
          ...
          if (data.highlights?.length > 0) {
            const highlights = data.highlights.map((highlight: string) => `<li>${highlight}</li>`).join("")
          ...
          if (data.keyUses?.length > 0) {
            const keyUses = data.keyUses.map((use: string) => `<li>${use}</li>`).join("")
          ...
          if (data.howToUse?.length > 0) {
            const howToUse = data.howToUse.map((step: string) => `<li>${step}</li>`).join("")
  ...
  return (
    <>
      <div className="w-full flex justify-end pb-4">
        <Button
          disabled={productDetailsMutation.isPending}
          className={`bg-blue-500 text-white dark:bg-blue-600 dark:hover:bg-blue-700 ${
            productDetailsMutation.isPending ? "opacity-50 cursor-not-allowed" : ""
          }`}
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Input placeholder={`Enter ${formSingular} Slug`} type="text" disabled {...field} />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
            ...
            <FormItem>
              ...
              <FormControl>
                <SelectWithSearch
                  ...
                  label="Category"
                  {...field}
                  newItemClick={handleNewItemClick}
                />
              </FormControl>
              <FormMessage />
            </FormItem>
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem className="col-span-3">
              ...
              <FormControl>
                <Textarea placeholder={`Enter ${formSingular} Description`} className="min-h-[100px]" {...field} />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem className="col-span-3">
              ...
              <FormControl>
                <Textarea placeholder={`Enter ${formSingular} Composition`} {...field} className="min-h-[100px]" />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="col-span-3">
                    ...
                    <FormControl>
                      <RichTextEditor value={field.value} onChange={(value: string) => field.onChange(value)} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
