...
function App() {
  ...
  const ordersTabs = [
    { path: 'view', activeWhen: 'view', label: 'View', Component: () => <Orders />, visible: true },
  ]

  const PaymentsTabs = [
    { path: 'view', activeWhen: 'view', label: 'View', Component: () => <PaymentView />, visible: true },
  ]

  const ContactTabs = [
    { path: 'view', activeWhen: 'view', label: 'View', Component: () => <ContactView />, visible: true },
  ]

  const UserTabs = [
    { path: 'view', activeWhen: 'view', label: 'View', Component: () => <UserView />, visible: true }
  ]
        ...
        {!isAuthenticated ? (
          ...
          <>
            ...
            <Route path="/admin" element={<AdminLayout />}>
              ...
              <Route path="orders" element={<SubPageLayout tabs={ordersTabs} defaultTab="view" title="Orders" />}>
                <Route index element={<Navigate to="view" replace />} />
                <Route path="view" element={<Orders />} />
                <Route path="view/:id" element={<OrderDetails />} />
              </Route>
              <Route path="users" element={<SubPageLayout tabs={UserTabs} defaultTab="view" title="Users" />}>
                <Route index element={<Navigate to="view" replace />} />
                <Route path="view" element={<UserView />} />
                <Route path="view/:id" element={<UserDetails />} />
              </Route>
              <Route path="payments" element={<SubPageLayout tabs={PaymentsTabs} defaultTab="view" title="Payments" />}>
                <Route index element={<Navigate to="view" replace />} />
                <Route path="view" element={<PaymentView />} />
                <Route path="view/:id" element={<PaymentDetail/>} />
              </Route>
              <Route path="contact" element={<SubPageLayout tabs={ContactTabs} defaultTab="view" title="Contacts" />}>
                <Route index element={<Navigate to="view" replace />} />
