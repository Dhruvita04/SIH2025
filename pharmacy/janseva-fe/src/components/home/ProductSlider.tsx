...
import { PRODUCTS } from '@/CONFIG/api-routes';
...
const ProductSlider: React.FC<ProductSliderProps> = ({ title, description, showTitle, halfSlider = false }) => {

    const getProducts = async () => {
        const response = await axiosInstance.get(`${PRODUCTS}?page=1&limit=15`);
    ...
    return (
        <div className="m-[12px] my-[20px] sm:m-[20px]">
            ...
            >
                ...
                {
                    productList && productList?.data.map((item: Product, _i: number) => {
